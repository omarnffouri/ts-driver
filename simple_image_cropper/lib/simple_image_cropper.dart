...
class SimpleImageCropperState extends State<SimpleImageCropper> {
  ...
  /// Crop the images based on the selected region and return Unit8List
  Future<Uint8List?> cropImageUint8List() async {
    if (imageEditor == null) return null;
    final ui.Image image = _imageInfo!.image;
    final Completer<Uint8List?> imgCompleter = Completer();
    final Region region = imageEditor!.region;
    ...
    ui.decodeImageFromPixels(
      ...
      final ByteData? byte =
          await image.toByteData(format: ui.ImageByteFormat.png);
      imgCompleter.complete(byte?.buffer.asUint8List());
    });
