...
class ShipmentsInfoWidget extends GetView<SettlementDetailsController> {
  const ShipmentsInfoWidget({super.key});

  @override
  Widget build(BuildContext context) {
    final shipments = controller.settlemetDetails.value.shipments;
      ...
      child: Column(
        ...
        children: [
          SettlementSectionHeader(
            title: 'Shipments',
            trailing: SettlementTotal(
                '\$${controller.settlemetDetails.value.paymentTotal?.decimalPattern()}'),
          ),
          Container(
...
String getPaymentRate(PaymentInfoEntity info) {
  ...
  switch (type.toLowerCase()) {
    case 'percentage':
      return '${num.parse(rate.toString()).toStringAsFixed(2)}%';
    case 'mileage':
      return '${num.parse(rate.toString()).toStringAsFixed(2)}Mi';
    case 'hourly':
...
class StepItem extends GetView<SettlementDetailsController> {
  final int index;
  final String time;
